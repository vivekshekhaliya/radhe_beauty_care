...
export default function Navbar() {
  ...
  // Scrollspy effect: Update active item during scroll when on the homepage
  useEffect(() => {
    ...
    const handleScrollspy = () => {
      const sections = [
        ...
        { id: "bridal-makeup", path: "/bridal-makeup" },
        { id: "testimonials", path: "/testimonials" },
        { id: "contact", path: "/contact" }
      ];

      let currentActive = "/";
      const scrollPosition = window.scrollY + 220; // Trigger trigger offset
  ...
  return (
    <div className="fixed top-0 left-0 w-full z-45 flex justify-center py-6 px-4 pointer-events-none">
      <header
        className={`w-full max-w-7xl px-6 py-4 flex items-center justify-between transition-all duration-500 pointer-events-auto ${isScrolled
      ...
      >
        ...
        <div className="flex items-center gap-3">
          <img src={logoImg} className="w-10 h-10 rounded-full object-cover border border-primary/30" alt="Logo" />
        ...
        <nav className="hidden lg:flex items-center space-x-6">
          {navLinks.map((link) => {
            const isActive = location.pathname === "/" && activeSection
            ...
            return (
              <Link
                key={link.name}
                to={link.path}
                className={`text-xs font-semibold uppercase tracking-wider font-sans py-2 relative transition-colors duration-250 ${isActive
              ...
              {navLinks.map((link) => {
                ...
                return (
                  <Link
                    key={link.name}
                    to={link.path}
                    className={`block px-4 py-3 rounded-[16px] text-base font-semibold transition-all ${isActive
